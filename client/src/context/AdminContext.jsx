...
export const AdminProvider = ({ children }) => {
  ...
  const fetchDepartments = useCallback(async (params = {}, force = false) => {
    if (
      !force &&
      isCacheValid("departments") &&
      !filtersChanged("departments", params)
    ) {
  ...
  const deleteDepartment = useCallback(
    async (id, transferId = null) => {
      try {
        ...
        updateCache(
          "departments",
          departments.filter((dept) => dept._id !== id)
        );
  ...
  const getDepartmentAnalytics = useCallback(async () => {
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to fetch department analytics"
      );
      throw err;
    } finally {
      setLoading(false);
  ...
  const fetchEmployees = useCallback(async (params = {}, force = false) => {
    if (
      ...
      !filtersChanged("employees", params)
    ) {
  ...
  const assignEmployeeDuties = useCallback(
    async (userId, departmentId, dutyIds, reason) => {
      try {
  ...
  const updateEmployeeStatus = useCallback(
    async (userId, status) => {
      try {
        const { data } = await adminService.updateEmployeeStatus(
          userId,
          status
        );
  ...
  const assignDepartmentAndDuties = useCallback(
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Failed to assign department and duties"
        );
        throw err;
      } finally {
        setLoading(false);
  ...
  const updateDepartmentAndDuties = useCallback(
    async (updateData) => {
      try {
        setLoading(true);
        const { data } = await adminService.updateDepartmentAndDuties(
          updateData
        );
        ...
        setError(
          err.response?.data?.message ||
            "Failed to update department and duties"
        );
        throw err;
      } finally {
        setLoading(false);
  ...
  const updateDuty = useCallback(async (id, updates) => {
    try {
      setLoading(true);
      const { data } = await adminService.updateDuty(id, updates);

      setDuties((prev) =>
        prev.map((duty) => (duty._id === id ? { ...duty, ...data } : duty))
      );
      clearCache("duties"); // Invalidate cache
  ...
  const deleteDuty = useCallback(async (id) => {
    try {
      setLoading(true);
      await adminService.deleteDuty(id);

      setDuties((prev) => prev.filter((duty) => duty._id !== id));
      clearCache("duties"); // Invalidate cache

      return true;
    } catch (err) {
      setError(err.response?.data?.message || "Failed to delete duty");
      throw err;
    } finally {
      setLoading(false);
    }
    ...
    async (id, status, feedback) => {
      try {
        ...
        setEmployeeTasks((prev) =>
          prev.map((task) =>
            task._id === id
              ? { ...task, status, feedback, reviewedAt: new Date() }
              : task
          )
        );
    ...
    async (id, decision) => {
      try {
        ...
        setAllLeaves((prev) =>
          prev.map((leave) =>
            leave._id === id
              ? { ...leave, status: decision.status, processedAt: new Date() }
              : leave
          )
        );
  ...
  const getLeaveAnalytics = useCallback(async () => {
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to fetch leave analytics"
      );
      throw err;
    }
  }, []);
  ...
  const getAdvancedLeaveAnalytics = useCallback(async (params = {}) => {
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to fetch advanced analytics"
      );
      throw err;
    }
  }, []);
  ...
  async (id, decision) => {
    try {
        ...
        prev.map((leave) =>
          leave._id === id
            ? { 
                ...
                decisionAt: new Date(),
                rejectionReason: decision.rejectionReason || leave.rejectionReason
              }
            : leave
        )
      );

      // Call the backend using the correct adminService method
      // The controller expects: PATCH /admin/leaves/:id with decision in body
      const { data } = await adminService.approveLeave(id, decision);

      // Update with actual response
      setAllLeaves((prev) =>
        prev.map((leave) =>
          leave._id === id ? { ...leave, ...data.leave } : leave
        )
      );
      ...
      // Revert optimistic update on error
      fetchAllLeaves({}, true); // Force refresh from server
  ...
  [fetchAllLeaves] // Changed dependency to avoid stale closure
);
  ...
  const fetchSalaries = useCallback(async (params = {}, force = false) => {
    if (
      !force &&
      isCacheValid("salaries") &&
      !filtersChanged("salaries", params)
    ) {
    ...
    try {
      ...
      const salariesData = Array.isArray(data)
        ? data
        : data?.data?.salaries || [];
  ...
  const fetchDepartmentHistory = useCallback(async (id) => {
    ...
    try {
      ...
      setDepartmentHistory(
        Array.isArray(data) ? data : data?.data?.history || []
      );
      setError(null);
    } catch (err) {
